...
class CabbyMM(BasePollerFT):
    def __init__(self, name, chassis, config):
        ...
        super(CabbyMM, self).__init__(name, chassis, config)

    def configure(self):
        super(CabbyMM, self).configure()
    ...
    def _saved_state_restore(self, saved_state):
        super(CabbyMM, self)._saved_state_restore(saved_state)
    ...
    def _saved_state_reset(self):
        super(CabbyMM, self)._saved_state_reset()
        self.last_taxii_run = None
    ...
    def _flush(self):
        self.last_taxii_run = None
        super(CabbyMM, self)._flush()

    def hup(self, source=None):
        LOG.info('%s - hup received, reload side config', self.name)
        self._load_side_config()
        super(CabbyMM, self).hup(source)
